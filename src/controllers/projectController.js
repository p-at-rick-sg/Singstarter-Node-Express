...
const getProjects = async (req, res) => {
  if (req.query.projectID) {
    ...
    try {
      ...
      console.error(err.message);
      return res.status(200).json({status: 'error', msg: 'failed to retrieve projectss'});
    }
  }
};
...
const getProjectPictures = async (req, res) => {
  try {
    ...
    console.error('Error: ', err);
    return res.status(400).json({status: 'error', msg: 'failed to get project pictures'});
  }
};
...
const addOrder = async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({status: 'ok', msg: 'failed to add order to database'});
  }
};
